void removeItem(enum itemType type) {
    for (int i = 0; i < player_items_n; i++) {
        if (player_items[i].item_type == type) {
...
void loadPlayerItems(const char *item_filename) {
    ...
    int item;
    char buffer[40];
    while ((fscanf(item_file, " %d", &item)) != EOF) {
        sprintf(buffer, "/items/%d.item", item);
        FILE *item = fopen(buffer, "r");

        struct item *cur = &player_items[player_items_n];

        cur->item_type = (enum itemType) item;
