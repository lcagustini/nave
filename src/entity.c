void doBasicFrame(int id) {
    entities[id].dir = vectorSubtract(entities[PLAYER_ID].pos, entities[id].pos);
    entities[id].dir.z = 0;
...
void doEntityFrame(int id) {
    ...
    entities[id].dir.z = 0.5*(entity_z.z - entities[id].pos.z);
    entities[id].pos = vectorAdd(entities[id].pos, entities[id].dir);
}
